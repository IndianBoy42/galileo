...
impl TileSchema {
    ...
    pub fn iter_tiles(&self, view: &MapView) -> Option<impl Iterator<Item = WrappingTileIndex>> {
        ...
        // Expand the bounding box slightly to ensure neighboring tiles are loaded
    ...
    /// Returns tile indices from the level above the given tile's z-level that cover
    /// the same area. Used as fallback when a tile is not yet loaded.
    ...
    /// Returns the LOD one z-level above the given.
    fn lod_over(&self, z: u32) -> Option<Lod> {
        if z == 0 || z as usize >= self.lods.len() {
            return None;
        }
        let resolution = *self.lods.get((z - 1) as usize)?;
        if resolution.is_finite() && resolution > 0.0 {
            Some(Lod {
                resolution,
                z_index: z - 1,
