    ...
    fn handle(&self, event: &UserEvent, map: &mut Map) -> EventPropagation {
        match event {
            ...
            UserEvent::Scroll(delta, mouse_event) => {
                let zoom = self.get_zoom(*delta, map.target_view().resolution());
...
impl MapController {
    ...
    pub fn get_zoom(&self, delta: f64, current_resolution: f64) -> f64 {
        let zoom = (self.config.zoom_speed + 1.0).powf(-delta);
        let target_resolution = current_resolution * zoom;
        if target_resolution > self.config.max_resolution {
            self.config.max_resolution / current_resolution
        } else if target_resolution < self.config.min_resolution {
            self.config.min_resolution / current_resolution
        } else {
            zoom
        }
    }
